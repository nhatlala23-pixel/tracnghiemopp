...
const rawPath = 'C:\\Users\\PC\\.gemini\\antigravity-ide\\brain\\0a860f6d-5f16-4fc5-b41a-cc5aa434fd44\\scratch\\chapter05_raw.txt';
const answerKeyPath = path.join(__dirname, '../src/data/answerKey.json');
const outputPath = path.join(__dirname, '../src/data/questions/chapter05.json');

const rawText = fs.readFileSync(rawPath, 'utf-8');
const answerKey = JSON.parse(fs.readFileSync(answerKeyPath, 'utf-8')).chapter05;
...
for (let i = 0; i < blocks.length; i++) {
  ...
  questions.push({
    id: `CH5-Q${String(qNum).padStart(3, '0')}`,
    chapter: 5,
    questionNumber: qNum,
    question: qText,
    options: {
...
console.log(`Parsed ${questions.length} questions for Chapter 5.`);
...
console.log('Successfully saved Chapter 5 to chapter05.json');
